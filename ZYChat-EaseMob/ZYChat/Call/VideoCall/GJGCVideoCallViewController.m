...
@interface GJGCVideoCallViewController ()<EMCallManagerDelegate>
{
    EMCallSession *_videosession;
    BOOL _isIncoming;
    NSTimer *_timeTimer;
...
@property (weak, nonatomic) IBOutlet EMCallLocalView *locationView;
@property (weak, nonatomic) IBOutlet EMCallRemoteView *remoteView;

@end

@implementation GJGCVideoCallViewController
...
- (IBAction)handFreeButtonAction:(UIButton *)sender {
    if ([sender.currentTitle isEqualToString:@"接听"]) {
        [[EMClient sharedClient].callManager answerIncomingCall:_videosession.sessionId];
    }
